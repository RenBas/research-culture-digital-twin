...
with st.expander("Step 1: Upload your CSV files", expanded=True):
    ...
    **Instructions:**
    ...
    """)
    col1, col2 = st.columns(2)
    with col1: survey_file = st.file_uploader("Survey CSV", type=["csv"], key="survey")
    with col2: metadata_file = st.file_uploader("Metadata CSV", type=["csv"], key="metadata")
    st.markdown("---")
    ...
    c1, c2 = st.columns(2)
    with c1: st.download_button("Survey Template", survey_template, "quarterly_survey_template.csv", "text/csv")
    with c2: st.download_button("Metadata Template", metadata_template, "research_metadata_template.csv", "text/csv")

# --- Main Area ---
if survey_file is not None and metadata_file is not None:
    survey_df_raw = pd.read_csv(survey_file); metadata_df_raw = pd.read_csv(metadata_file)
    ...
    if survey_error: st.error(f"Survey error: {survey_error}")
        ...
        actual_count = len(school_info); total_teachers = metadata_df['teacher_name'].nunique()
        state_changed = False
        if st.session_state.num_schools != actual_count: st.session_state.num_schools = actual_count; state_changed = True
        if st.session_state.total_teachers != total_teachers: st.session_state.total_teachers = total_teachers; state_changed = True
        ...
        if not df_show.empty: st.dataframe(df_show[['teacher_name', 'year_undertaken', 'title', 'theme', 'status', 'utilized_by_school']].head(10))
        else: st.info("No research outputs for this school.")
        ...
        else: st.info("No survey data for current quarter.")
        ...
        if baseline_btn:
            ...
            else: st.warning("No survey data to generate baseline.")
        ...
        if 'calibrated_coeff' not in st.session_state:
            with st.spinner("Calibrating model coefficients..."):
                ...
                if coeff: st.success(calib_msg); st.session_state.calibrated_coeff = coeff
                else: st.warning(calib_msg); st.session_state.calibrated_coeff = None
        ...
        if run_btn:
            ...
            for m in range(1, duration+1):
                if use_survey: apply_survey_override(st.session_state.sim.agents, survey_df, m)
            ...
            if mc_enabled:
                with st.spinner(f"Running {mc_runs} Monte Carlo simulations..."):
                    ...
                    st.session_state.mc_data = mc_data; st.session_state.mc_info = mc_info
            st.rerun()

        if step_btn:
            target = st.session_state.current_month + 1
            if use_survey: apply_survey_override(st.session_state.sim.agents, survey_df, target)
        ...
        if st.session_state.total_months > 0:
            ...
            if hist and agent:
                ...
                with st.expander("Cycle vs Research Outputs"): cycle_research_correlation(agent, metadata_df, selected_school_id, dark_mode)
                with st.expander("Division-Level Analysis"): div_metrics = division_level_analysis(metadata_df, st.session_state.history, st.session_state.sim.agents, dark_mode)
                with st.expander("Comparative School Analysis"):
                    selected_comparison = st.multiselect("Select schools to compare", options=school_ids, format_func=lambda x: id_to_label[x])
                ...
                if not st.session_state.get('sensitivity_fig'):
                    with st.spinner("Computing sensitivity analysis..."):
                        fig_tornado, sensitivity_info = run_sensitivity(Simulation, agent_params, school_ids, levers, duration, use_survey, survey_df, metadata_df, selected_school_id)
                        st.session_state.sensitivity_fig = fig_tornado; st.session_state.sensitivity_info = sensitivity_info
                else: fig_tornado = st.session_state.sensitivity_fig; sensitivity_info = st.session_state.sensitivity_info
                ...
                with st.expander("Sensitivity Analysis (Tornado)"):
                    ...
                    if sensitivity_info: st.markdown(sensitivity_info)
                ...
                if agent.cycle_count >= 2: cycle_text = f"has completed {agent.cycle_count} full cycles, indicating a self-sustaining research culture."
                elif agent.cycle_count == 1: cycle_text = "has completed one full cycle, demonstrating initial sustainability."
                else: cycle_text = "has not yet completed any full cycle."
                if agent.current_milestone == 0: milestone_progress = "is at the very beginning of the journey."
                elif agent.current_milestone <= 2: milestone_progress = "has moved beyond initial readiness but remains in early capacity‑building phases."
                elif agent.current_milestone <= 4: milestone_progress = "has established structured support and is embedding research into institutional practice."
                else: milestone_progress = "is realising tangible impact and is approaching or has achieved cyclical sustainability."
                ...
                if ('baseline_synopsis' in st.session_state and 'baseline_survey_row' in st.session_state):
                    bs = st.session_state.baseline_synopsis; baseline_vals = st.session_state.baseline_survey_row; gaps = bs['gaps']
                    if gaps:
                        st.markdown("#### Baseline vs Simulation Comparison (Critical Gaps)")
                        table_data = []; baseline_std_devs = st.session_state.get('baseline_std_devs', {})
                        for var in gaps:
                            base_val = baseline_vals[var]; sim_val = getattr(agent, var); diff = sim_val - base_val
                            status = "Improving" if diff > 0.01 else ("Regressing" if diff < -0.01 else "Stable")
                            std_dev = baseline_std_devs.get(var, 0.1)
                            if abs(diff) >= 0.10: significance = "Both statistically and practically significant"
                            elif abs(diff) >= 0.5 * std_dev: significance = "Statistically significant, but limited practical impact"
                            else: significance = "Not significant (within normal variability)"
                            table_data.append({"Critical Gap": VAR_FULL_NAMES[var], "Baseline": f"{base_val:.2f}", "Simulation": f"{sim_val:.2f}", "Status": status, "Significance": significance})
                ...
                if early_percent == 100: sustainability_text = "All schools are in early milestones; foundational capacity‑building is the priority."
                elif early_percent >= 75: sustainability_text = f"The vast majority ({early_percent:.1f}%) are in early milestones; urgent interventions needed."
                elif early_percent >= 50: sustainability_text = f"More than half ({early_percent:.1f}%) are in early milestones; targeted policy support may accelerate progress."
                elif early_percent > 0: sustainability_text = f"{early_percent:.1f}% remain in early milestones; continued efforts are required."
                else: sustainability_text = "No schools are in early milestones; the division exhibits a strong, advanced research culture."
                ...
                level_avg = classify_rcsi(avg_rcsi); total_cycles = sum(a.cycle_count for a in st.session_state.sim.agents)
                ...
                top_div_teacher = div_metrics.get('top_div_teacher', 'N/A'); top_div_school = div_metrics.get('top_div_school', 'N/A')
                top_div_outputs = div_metrics.get('top_div_outputs', 0)
                bottleneck_milestone = div_metrics.get('bottleneck_milestone', 'N/A'); bottleneck_time = div_metrics.get('bottleneck_time', 0)
                ...
                if not metadata_df.empty and 'upload_date' in metadata_df.columns:
                    ...
                    if len(div_timeline) >= 2:
                        if div_timeline.iloc[-1] > div_timeline.iloc[-2]: output_trend_div = "The division's research output is increasing over time."
                        elif div_timeline.iloc[-1] < div_timeline.iloc[-2]: output_trend_div = "The division's research output is declining over time."
                        else: output_trend_div = "The division's research output has remained stable."
            ...
            if export_btn:
                ...
                for a in st.session_state.sim.agents:
                    ...
                    for t in range(len(h['month'])):
                        ...
                        for v in VARIABLES: row[v] = h[v][t]
                        all_data.append(row)
                df_hist = pd.DataFrame(all_data)
                cycle_records = []
                for a in st.session_state.sim.agents:
                    for rec in a.cycle_improvements:
                        cycle_records.append({'school_id': a.real_id, 'cycle_number': rec.cycle_number, 'total_improvement': rec.total_improvement, 'completion_month': rec.completion_month})
