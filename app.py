...
# ------------------------------------------------------------
# Phase 2 functions – now accept school_ids to assign real_id
# ------------------------------------------------------------
...
# ------------------- Modified sensitivity & MC (now accept school_ids) -------------------
...
# ------------------------------------------------------------
# Streamlit UI – only ONE dark mode checkbox
# ------------------------------------------------------------
...
with st.sidebar:
    # dark_mode and apply_theme already handled by Phase 1 – do NOT duplicate them here
        ...
        if run_btn:
            ...
            if mc_enabled:
                with st.spinner(f"Running {mc_runs} Monte Carlo simulations..."):
                    mc_data = monte_carlo_sim(mc_runs, Simulation, agent_params, school_ids, levers, duration, use_survey, survey_df, metadata_df, selected_school_id)
            ...
            if hist and agent:
                ...
                with st.expander("🎯 Sensitivity Analysis (Tornado)"):
                    with st.spinner("Computing sensitivity..."):
                        fig_tornado = run_sensitivity(Simulation, agent_params, school_ids, levers, duration, use_survey, survey_df, metadata_df, selected_school_id)
